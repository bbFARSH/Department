...
class SalaryWorker :public Human
{
	string post;
	unsigned int experience;
	double time_worked;
...
void main()
{
	...
	Human* group[] =
	{
		new Worker("Pinkman", "Jessie", 22, "SalaryWorker", 12, 15000),
		new SalaryWorker("White", "Walter", 50, "Worker", 15, 180, 1000),
		new Worker("Diaz", "Ricardo", 55, "SalaryWorker", 34, 30000),
		new SalaryWorker("Eistein", "Albert", 54, "Worker", 25, 180, 1500),
	};
...
Human* HumanFactory(const std::string& human_type)
{
	if (human_type.find("class Accounting") != string::npos) return new Worker("last_name", "first_name", 0, "spec", 0, 0);
	if (human_type.find("class Logistics") != string::npos) return new SalaryWorker("last_name", "first_name", 0, "spec", 0, 0, 0);
	return nullptr;
}